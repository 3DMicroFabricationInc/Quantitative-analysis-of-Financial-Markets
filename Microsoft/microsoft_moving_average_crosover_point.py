...
def display_plot():
    ...
    plt.plot(df['Close'], label='MSFT Close Price', color='blue')
    ...
    plt.title("Microsoft Stock: Close vs 20-day MA")
    plt.xlabel("Date")
...
if msft is None:
    ...
    df.to_csv("microsoft_stock_data.csv")  # Save to CSV for debugging
    ...
    # Plot with crossover markers
    plt.figure(figsize=(12,6))
    plt.plot(msft['Close'], label='MSFT Close Price', color='blue')
    plt.plot(msft['MA20'], label='20-day MA', color='red')

# Mark crossover points
plt.scatter(crossovers.index, crossovers['Close'], 
            color='black', marker='o', label='Crossovers')

plt.title("Microsoft Stock: Close vs 20-day MA Crossovers")
plt.xlabel("Date")
plt.ylabel("Price (USD)")
plt.legend()
plt.grid(True)
